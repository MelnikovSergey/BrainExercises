...
{
	// Попросили бросить исключение. Я не хотел ;)
	if ($a < 0 || $b < 0) {
		throw new Exception('Один из аргументов не валиден.');
	}

	$range = range($a, $b); $veryIntegerUnit = 1; $result = [];

	for ($i = 0; $i < count($range); $i++) {  
		
		$stateMashineVariable = null;

		for ($veryIntegerNumber = 1; $veryIntegerNumber <= $range[$i]; $veryIntegerNumber++) { 

			if (($range[$i] % $veryIntegerNumber) === 0) {
				if ($veryIntegerNumber === $veryIntegerUnit) continue;
				if ($veryIntegerNumber === $range[$i]) continue;

				$stateMashineVariable .= $veryIntegerNumber;
			} 
		} 

		if ($range[$i] !== $veryIntegerUnit && $stateMashineVariable === null) { 
			$result[$i] = $range[$i];
		} 
	}
...
function createTrapeze($a) {
	...
	foreach (array_chunk($a, count($keys)) as $value) {
		$result[] = array_combine(array_slice($keys, 0, count($value)), $value);
	}
